    ...
    return (
      <g transform={`translate(${x},${y})`}>
        <text x={0} y={0} dy={16} textAnchor="end" fill="#666" style={{fontSize: "12px"}}
              transform="rotate(-25)">{payload.value}</text>
      </g>
    );
  }
}
  ...
  renderHomeView() {
    ...
    return (
      <div className="main-content"
           ref={div => this.setMainContent(div)}
      >

        <Header chartFilterStartDate={chartFilterStartDate}
        ...
        {lineKeys.length ? (
          ...
          : (<div><h3 style={{textAlign: 'center'}}>{this.props.translate.noChartsData}</h3></div>)
        }
